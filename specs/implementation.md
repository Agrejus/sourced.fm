...
- **Voice prompt wavs:** the processor conditions on one reference wav per
  unique speaker, ordered to match the labels above. All three are fork demo
  clips: `speech/voices/host.wav` (= `demo/voices/en-Alice_woman.wav`, female,
  Maya), `speech/voices/expert.wav` (= `demo/voices/en-Frank_man.wav`, male,
  Sam), `speech/voices/critic.wav` (= `demo/voices/in-Samuel_man.wav`, male,
  Theo). Samuel was chosen by ear over a deeper American alternative for reading
  as more inquisitive, which is CRITIC's job; its Indian-English accent is
  deliberate. `speech/make_critic_voice.py` regenerates a Kokoro alternative
  (`am_onyx`) that measures better but was not preferred.

  EXPERT and CRITIC are both male, so their **rendered pitches** must not
  converge or the listener hears one person arguing with himself. Judge that on
  median F0 and nothing else — two apparently better measures were tested and
  discarded (2026-08-04): reference-clip spectral similarity does not predict the
  render (two references 3.4x apart rendered 0.0425 vs 0.0426 from Frank), and
  rendered timbre distance is unreliable (in the same test Maya scored *closer*
  to Sam than CRITIC, though she is female and 60 Hz away). Measured across full
  episodes: Samuel ~4 Hz from Frank, `am_onyx` 11-26 Hz. Samuel ships anyway —
  character beat separation, and turn structure (CRITIC asks short questions,
  EXPERT gives long answers) carries much of the load. The fork's
  `en-Carter_man.wav` stays rejected for rendering 2.6 Hz from Frank. Verify any
  replacement on a real multi-minute excerpt, never a 60-second smoke test, which
  overstates separation badly (Samuel measured 16.8 Hz on 61 seconds and 4.1 Hz
  across a full hour). These files are part of the repo, not env config;
  provenance is recorded in the `speech/models.py` docstring.
- **Processor + generate:**
...
```
You are preparing source material for a podcast that explains things properly.
...
```
You research ONE concept so a podcast can explain it properly. The user names
