...
- **Voice prompt wavs:** the processor conditions on one reference wav per
  unique speaker, ordered to match the labels above. `speech/voices/host.wav`
  (= fork `demo/voices/en-Alice_woman.wav`, female, Maya) and
  `speech/voices/expert.wav` (= fork `demo/voices/en-Frank_man.wav`, male, Sam)
  are fork demo clips. `speech/voices/critic.wav` (male, Ray) is generated by
  `speech/make_critic_voice.py` with Kokoro `am_onyx` — reproducible, and no
  third-party audio.
  ...
  to Sam than Ray, though she is female and 60 Hz away). `am_onyx` renders ~19 Hz
  below Frank, about twice the next candidate's gap; the fork's
  `en-Carter_man.wav` was rejected for rendering 2.6 Hz from him. Verify any
  replacement on a real multi-minute excerpt, never a 60-second smoke test — that
  overstates separation badly (one candidate measured 16.8 Hz on 61 seconds and
  4.1 Hz across a full hour). These files are part of the repo, not env config;
  provenance is recorded in the `speech/models.py` docstring. Get the M1 mp3 quality check approved before
