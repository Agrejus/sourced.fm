...
Voice prompt wavs in speech/voices/:
  ...
  critic.wav <- generated by make_critic_voice.py    (male,   CRITIC — Ray)

The first two are vibevoice-community/VibeVoice demo clips. Ray's is generated
locally with Kokoro (am_onyx), which is reproducible and needs no third-party
attribution.
...
- **Spectral/timbre distance between rendered speakers is unreliable.** In the
  same test Maya scored *closer* to Sam than Ray did, which is plainly false —
  she is female and 60 Hz away. It tracks content and run-to-run variance more
  than speaker identity.

What holds up: am_onyx renders ~19 Hz below Frank, about twice the next
candidate's gap. The fork's en-Carter_man.wav was rejected for rendering 2.6 Hz
from Frank — effectively the same voice. Verify any replacement by rendering a
real multi-minute excerpt and listening to it; a 60-second smoke test overstates
separation badly (Samuel measured 16.8 Hz on 61 seconds and 4.1 Hz across a full
hour).
"""
